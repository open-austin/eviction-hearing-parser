...
def get_before_after_date(sep):
    ...
    return beforedate_str, afterdate_str
...
def print_and_email(message, subject):
...
def perform_task_and_catch_errors(task_function, task_name):
    ...
    for tries in range(1, 6):
        try:
            task_function()
            print(f"Finished {task_name} in {time.time() - before} seconds.")
            return
        except Exception as error:
            print(str(error), f"Unanticipated Error {task_name} on attempt {tries} of 5.")
    print_and_email(f"{task_name} failed on every attempt. Check Heroku logs for more details.", f"{task_name} failed")
...
sched.add_job(scrape_filings, 'interval', minutes=5, start_date='2020-09-09 17:13:00', timezone='US/Eastern')
sched.add_job(scrape_settings, 'interval', minutes=5, start_date='2020-09-10 18:00:00', timezone='US/Eastern')
# sched.add_job(scrape_settings, 'interval', weeks=1, start_date='2020-09-10 18:00:00', timezone='US/Eastern')
# sched.add_job(scrape_settings, 'interval', weeks=1, start_date='2020-09-10 18:00:00', timezone='US/Eastern')

sched.add_job(fake_task, 'interval', minutes=5, start_date='2020-09-10 18:00:00', timezone='US/Eastern')
sched.start()
